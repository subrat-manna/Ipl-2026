...
var PLAYER_NAMES = [
  ...
  "Samrat","Nihar","Naruto","CheaterAbhii","Manna","Sipun","Raka@kaaka","Jammydips","Sanu","Arman@H1B"
];
...
function renderNameChips(filter) {
  ...
  grid.innerHTML = filtered.map(function(name) {
    ...
    var border = isSelected ? 'var(--gold)' : 'rgba(255,255,255,0.12)';
    var bg     = isSelected ? 'rgba(255,215,0,0.15)' : 'rgba(255,255,255,0.04)';
    var color  = isSelected ? 'var(--gold)' : 'var(--text)';
    var shadow = isSelected ? 'box-shadow:0 0 12px rgba(255,215,0,0.3);' : '';
    var style  = 'padding:10px 16px;border-radius:30px;font-family:Rajdhani;font-size:15px;font-weight:700;border:2px solid '+border+';background:'+bg+';color:'+color+';cursor:pointer;transition:all 0.2s;'+shadow;
    return '<button type="button" onclick="selectName(this.dataset.name)" data-name="'+name+'" style="'+style+'">'+name+'</button>';
  }).join("");
}
